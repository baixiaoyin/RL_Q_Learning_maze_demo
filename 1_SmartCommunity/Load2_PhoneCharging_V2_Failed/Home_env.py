...
class HEMS_Env(object) :
    ...
    def step(self, action):
        ...
        if action == 'off':
            self.action = 'off'
            if s == TotalTime - Duration + self.count - 1:
                s_ = "Terminal"
                #R = DeviceEnergy * (sum(Eprice_list[s + 1:]))
                Rt = DeviceEnergy * (sum(Eprice_list[s+1:]))
                self.R = self.R + Rt
                self.done = True
            else:
                #R = 100  #??
            ...
            self.action = 'on'
            if self.count == Duration - 1:
                s_ = "Terminal"
                #R = DeviceEnergy * (Eprice_list[s:s+1])
                Rt = DeviceEnergy * (sum(Eprice_list[s:s+1]))
                self.R = self.R + Rt
        ...
        return s_, R, self.done, self.action
